...
from sqlalchemy import Column, String, create_engine, Integer, Float, DateTime, Date, ForeignKey, func
...
class coupons(Base):
    # 表的名字:
    __tablename__ = 'couponstest'

    # 表的结构:
    dealnumber = Column(Integer,primary_key=True,unique=True,nullable=False)
    description = Column(String(40),nullable=False)
    location = Column(String(15),nullable=False)
    dealprice=Column(Float,nullable=False)
...
@app.route('/search_100', methods=['GET', 'POST'])
def search_100():
    return render_template('search_100.html', new_coupon1=
    session1.query(coupons.dealnumber,coupons.description).
                           having(func.count(Sign.dealnumber)>=100).join(Sign, Sign.dealnumber == coupons.dealnumber)
                           .group_by(coupons.dealnumber).order_by(coupons.dealnumber).all())


@app.route('/mostpopulardeal', methods=['GET', 'POST'])
def mostpopulardeal():
    return render_template('mostpopulardeal.html', new_coupon1=
    session1.query(coupons.dealnumber,coupons.description,func.count(Sign.dealnumber).label("number")).
                           join(Sign, Sign.dealnumber == coupons.dealnumber)
                           .group_by(coupons.dealnumber).order_by(func.count(Sign.dealnumber).desc()).all())
